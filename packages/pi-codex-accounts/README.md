...
- **자동 전환·재개**: `429 / rate limit / quota` 에러에서 실제 소진된 `5시간/7일` 창의 `resetAt`까지 쿨다운을 걸고, 사용 가능한 계정으로 전환해 중단 지점부터 자동으로 이어간다. 전역 lease로 동시에 한 세션만 자동 재개하며 다른 세션은 계정만 전환해 연쇄 소진을 막는다
