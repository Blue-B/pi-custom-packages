...
- **현재 실패 응답만 재개**: 같은 세션, 모델, 실패 메시지가 그대로인지 확인한다. 과거 사용자 요청을 찾아 복사하지 않는다. 실패 메시지 ID를 담은 짧은 확장 안내만 추가하고 기존 대화를 이어간다. 새 입력, 대기 중인 메시지, 중단, 세션이나 대화 경로 변경이 있으면 재개하지 않는다
