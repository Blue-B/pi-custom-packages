...
function findScript(name: string): string {
  ...
  throw new Error(`pi-winshot: cannot find ${name}; tried ${SCRIPT_DIR_CANDIDATES.join(", ")}`);
}
...
function wslToWin(p: string): string {
  ...
  if (p.startsWith("/")) {
    try {
      const win = execFileSync("wslpath", ["-w", p], { encoding: "utf8" }).trim();
      if (win) return win;
    } catch {
...
function runPS(scriptWinPath: string, args: string[], signal?: AbortSignal): Promise<PSResult> {
...
const EMBED_DOWNSCALE = process.env.WINSHOT_NO_DOWNSCALE !== "1" && FFMPEG !== null;

async function readImageContent(linuxPath: string): Promise<{ type: "image"; data: string; mimeType: string }> {
  if (EMBED_DOWNSCALE) {
    const out = join(tmpdir(), `winshot_embed_${Date.now()}_${Math.random().toString(36).slice(2, 8)}.jpg`);
    try {
      execFileSync(FFMPEG!, [
        "-y", "-loglevel", "error", "-i", linuxPath,
        // Fit inside an EMBED_MAX_EDGE square, preserving aspect ratio, never upscaling.
        "-vf", `scale=w=min(${EMBED_MAX_EDGE}\\,iw):h=min(${EMBED_MAX_EDGE}\\,ih):force_original_aspect_ratio=decrease`,
        "-q:v", "5", out,
...
export default function winshot(pi: ExtensionAPI): void {
  if (!IS_WSL) {
    ...
    pi.on("session_start", (_e, ctx) => {
      if (ctx.hasUI) {
        ctx.ui.notify("pi-winshot is intended for WSL. powershell.exe must be reachable.", "warning");
      }
    });
  }
  ...
  pi.registerTool({
    ...
    execute: async (_id, _params, signal) => {
      const r = await runPS(wslToWin(listPs), ["-What", "windows", "-Json"], signal);
      if (r.code !== 0) throw new Error(`list windows failed: ${r.stderr || r.stdout}`);
      const data = parseJsonLine(r.stdout);
      const text = (Array.isArray(data) ? data : [data])
        .map((w: any) => `${w.handle} ${w.minimized ? "[min]" : "     "} ${String(w.w).padStart(5)}x${String(w.h).padEnd(5)}  ${w.title}`)
        .join("\n");
      return {
  ...
  pi.registerTool({
    ...
    label: "List monitors",
    description: "List connected monitors with their virtual-desktop coordinates and primary flag.",
    parameters: Type.Object({}),
    executionMode: "parallel",
    execute: async (_id, _params, signal) => {
      const r = await runPS(wslToWin(listPs), ["-What", "monitors", "-Json"], signal);
      if (r.code !== 0) throw new Error(`list monitors failed: ${r.stderr || r.stdout}`);
      const data = parseJsonLine(r.stdout);
      const arr = Array.isArray(data) ? data : [data];
      const text = arr
        .map((m: any) => `monitor[${m.index}] primary=${m.primary} x=${m.x} y=${m.y} w=${m.w} h=${m.h}`)
        .join("\n");
      return {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      title: Type.Optional(Type.String({ description: "case-insensitive substring of the window title" })),
      bring_to_front: Type.Optional(Type.Boolean({ default: false })),
      out: Type.Optional(Type.String({ description: "output path; accepts WSL (/mnt/c/...) or Windows (C:\\...) path" })),
      return_image: Type.Optional(Type.Boolean({ default: true, description: "embed the PNG in the tool result so the model can see it" })),
    }),
    ...
    execute: async (_id, params, signal) => {
      ...
      const outLinux = params.out
        ? (params.out.startsWith("/") ? params.out : winToWsl(params.out))
        : join(outDir, uniqName(`cap_${params.mode}`));
      const outWin = wslToWin(outLinux);

      const args: string[] = ["-Mode", params.mode, "-Out", shellQuote(outWin), "-Json"];
      if (params.mode === "region") {
        if (params.x == null || params.y == null || params.w == null || params.h == null) {
          throw new Error("region mode requires x, y, w, h");
        }
        args.push("-X", String(params.x), "-Y", String(params.y), "-W", String(params.w), "-H", String(params.h));
      }
      if (params.mode === "monitor") {
        if (params.monitor == null) throw new Error("monitor mode requires monitor index");
      ...
      if (r.code !== 0) throw new Error(`capture failed: ${r.stderr || r.stdout}`);
  ...
  pi.registerTool({
    ...
    executionMode: "sequential",
    execute: async (_id, params, signal) => {
      const inLinux = params.in.startsWith("/") ? params.in : winToWsl(params.in);
      ...
      const args = [
        "-Op", "crop",
        "-In", shellQuote(wslToWin(inLinux)),
        "-Out", shellQuote(wslToWin(outLinux)),
        "-X", String(params.x), "-Y", String(params.y), "-W", String(params.w), "-H", String(params.h),
        "-Json",
      ];
      const r = await runPS(wslToWin(editPs), args, signal);
      if (r.code !== 0) throw new Error(`crop failed: ${r.stderr || r.stdout}`);
      const meta = parseJsonLine(r.stdout);
      const content: any[] = [{ type: "text", text: `Cropped to ${meta.w}x${meta.h} at (${meta.x},${meta.y})\nsaved: ${outLinux}` }];
      if (params.return_image !== false) content.push(await readImageContent(outLinux));
      return { content, details: { ...meta, out_linux: outLinux } };
    },
  });
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      style: Type.Optional(Type.Union([Type.Literal("black"), Type.Literal("blur"), Type.Literal("pixelate")], { default: "black" } as any)),
      pixel: Type.Optional(Type.Integer({ minimum: 2, description: "pixelate block size", default: 16 } as any)),
      blur_radius: Type.Optional(Type.Integer({ minimum: 2, default: 12 } as any)),
      out: Type.Optional(Type.String()),
      return_image: Type.Optional(Type.Boolean({ default: true })),
    }),
    executionMode: "sequential",
    execute: async (_id, params, signal) => {
      ...
      const regionSpec = params.regions.map((r: any) => `${r.x},${r.y},${r.w},${r.h}`).join(";");
      const style = (params as any).style ?? "black";
      const args = [
        "-Op", "mask",
        "-In", shellQuote(wslToWin(inLinux)),
        "-Out", shellQuote(wslToWin(outLinux)),
        "-Regions", shellQuote(regionSpec),
        "-MaskStyle", style,
        "-Json",
      ];
      if ((params as any).pixel != null) args.push("-Pixel", String((params as any).pixel));
      if ((params as any).blur_radius != null) args.push("-BlurRadius", String((params as any).blur_radius));
      const r = await runPS(wslToWin(editPs), args, signal);
      if (r.code !== 0) throw new Error(`mask failed: ${r.stderr || r.stdout}`);
      const meta = parseJsonLine(r.stdout);
      const content: any[] = [{ type: "text", text: `Masked ${meta.count} region(s) with style=${meta.style}\nsaved: ${outLinux}` }];
      if (params.return_image !== false) content.push(await readImageContent(outLinux));
      return { content, details: { ...meta, out_linux: outLinux } };
    },
  });
  ...
  pi.registerTool({
    ...
    execute: async (_id, params, signal) => {
      ...
      const r = await runPS(wslToWin(editPs), args, signal);
      if (r.code !== 0) throw new Error(`resize failed: ${r.stderr || r.stdout}`);
      const meta = parseJsonLine(r.stdout);
      const content: any[] = [{ type: "text", text: `Resized ${meta.from_w}x${meta.from_h} -> ${meta.w}x${meta.h}\nsaved: ${outLinux}` }];
      if (params.return_image !== false) content.push(await readImageContent(outLinux));
      return { content, details: { ...meta, out_linux: outLinux } };
    },
  });
  ...
  pi.registerTool({
    ...
    description: "Report the width/height of a PNG. Use this before crop/mask to ground your coordinate math.",
    parameters: Type.Object({ in: Type.String() }),
    executionMode: "parallel",
    execute: async (_id, params, signal) => {
      const inLinux = params.in.startsWith("/") ? params.in : winToWsl(params.in);
      if (!existsSync(inLinux)) throw new Error(`input not found: ${inLinux}`);
      const args = ["-Op", "info", "-In", shellQuote(wslToWin(inLinux)), "-Json"];
      const r = await runPS(wslToWin(editPs), args, signal);
  ...
  pi.registerCommand("winshot", {
    description: "Capture the Windows screen. Usage: /winshot [full|active|list|window <q>]",
    handler: async (args, _ctx) => {
      const a = (args ?? "").trim();
      const parts = a.split(/\s+/).filter(Boolean);
      const sub = parts[0] || "full";
      if (sub === "list") {
        await pi.sendMessage({ customType: "user-text" as any, content: "Use winshot_list_windows tool and show me the result.", display: false });
        return;
      }
      if (sub === "window") {
