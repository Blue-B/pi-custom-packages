...
import { readFileSync, writeFileSync, copyFileSync, renameSync, statSync, utimesSync, readdirSync } from "node:fs";
...
function estTokens(o) {
  ...
  let n = 0;
  for (const b of c) {
    if (b.type === "image" || b.type === "image_url" || b.type === "input_image") n += 50000;
    else if (b.type === "thinking") n += Math.ceil((b.thinking || "").length / 4);
    else if (b.type === "text") n += Math.ceil((b.text || "").length / 4);
    else if (b.type === "tool_use") n += Math.ceil(JSON.stringify(b.input || {}).length / 4) + 10;
    else if (b.type === "tool_result") n += Math.ceil(JSON.stringify(b.content || "").length / 4) + 10;
...
function plan(file) {
  ...
  for (let i = 0; i < rawLines.length; i++) {
    const ln = rawLines[i];
    if (!ln.trim()) continue;
    try { objs.push(JSON.parse(ln)); lineIdxOfObj.push(i); } catch { /* skip */ }
  }
  ...
  // ---- decide trim ----
  let trimRec = null, effectiveLive = realLive, boundaryTs = null;
  if (realLive > CEILING) {
    const tailTok = new Array(live.length).fill(0);
    let cum = 0;
    for (let i = live.length - 1; i >= 0; i--) { cum += live[i].type === "message" ? estTokens(live[i]) : 0; tailTok[i] = cum; }
    let pick = -1;
    for (let i = 0; i < live.length; i++) if (isUserPrompt(live[i]) && tailTok[i] <= TARGET_TAIL) { pick = i; break; }
    if (pick < 0) for (let i = live.length - 1; i >= 0; i--) if (isUserPrompt(live[i])) { pick = i; break; }
    if (pick < 0) return { skip: "no-clean-boundary", realLive };
    const boundary = live[pick];
    const kept = live.slice(pick);
    const toolUse = new Set();
    for (const o of kept) { const m = o.message; if (m && Array.isArray(m.content)) for (const b of m.content) if (b.type === "tool_use") toolUse.add(b.id); }
    for (const o of kept) { const m = o.message; if (m && Array.isArray(m.content)) for (const b of m.content) if (b.type === "tool_result" && !toolUse.has(b.tool_use_id)) return { skip: "orphan-at-boundary", realLive }; }
    effectiveLive = summaryTok + tailTok[pick];
    boundaryTs = boundary.timestamp;
    const used = new Set(objs.map((o) => o.id));
    let nid; do { nid = Math.floor(Math.random() * 0xffffffff).toString(16).padStart(8, "0"); } while (used.has(nid));
...
`;
    const rec = {
      type: "compaction", id: nid, parentId: objs[objs.length - 1].id, timestamp: new Date().toISOString(),
      summary: (last.summary || "") + note, firstKeptEntryId: boundary.id, tokensBefore: realLive,
      details: { readFiles: [], modifiedFiles: [] }, fromHook: false,
    };
  ...
  let usageEdit = null;
  let lastAsstRaw = -1, lastAsstObj = null;
  for (let k = objs.length - 1; k >= 0; k--) {
    const m = objs[k].message;
    if (m && m.role === "assistant" && m.usage) { lastAsstRaw = lineIdxOfObj[k]; lastAsstObj = objs[k]; break; }
  }
  if (lastAsstObj) {
    ...
    if ((trimRec || isLandmine) && Math.abs(cur - desired) > USAGE_TOL) {
      ...
      nu.totalTokens = (nu.input || 0) + (u.output || 0) + desired + (u.cacheWrite || 0);
      const clone = JSON.parse(JSON.stringify(lastAsstObj));
      clone.message.usage = nu;
      usageEdit = { rawIdx: lastAsstRaw, line: JSON.stringify(clone), from: cur, to: desired };
    }
  }

  if (!trimRec && !usageEdit) return { skip: "healthy", realLive };
  return { raw, rawLines, realLive, effectiveLive, boundaryTs, trimRec, usageEdit };
}

function apply(file, p) {
  const st = statSync(file);
  const bak = file + ".bak-livesize-" + new Date().toISOString().replace(/[:.]/g, "").slice(0, 15);
  ...
  let bad = 0;
  for (const ln of out.split("\n")) { if (!ln.trim()) continue; try { JSON.parse(ln); } catch { bad++; } }
  if (bad) throw new Error("validation failed: " + bad + " bad lines");
  const tmp = file + ".tmp-livesize";
  writeFileSync(tmp, out);
  renameSync(tmp, file);
  try { utimesSync(file, st.atime, st.mtime); } catch {}
...
function collectAll() {
  const out = [];
  let dirs;
  try { dirs = readdirSync(SESS_DIR, { withFileTypes: true }); } catch { return out; }
  for (const d of dirs) {
    ...
    for (const f of readdirSync(sub)) {
      ...
      try { if (statSync(pth).size < MIN_FILE_MB * 1024 * 1024) continue; } catch { continue; }
      out.push(pth);
    }
  }
  return out;
...
if (!targets.length) { console.log("usage: cap-session-livesize.mjs (--all | <file.jsonl>) [--apply]"); process.exit(1); }

console.log(`mode=${APPLY ? "APPLY" : "DRY-RUN"} ceiling=${CEILING} target=${TARGET_TAIL} tol=${USAGE_TOL} files=${targets.length}\n`);
let acted = 0, healed = 0;
for (const file of targets) {
  let p;
  try { p = plan(file); } catch (e) { console.log("ERR ", file.split("/").pop(), e.message); continue; }
  if (p.skip === "healthy" || p.skip === "no-compaction-record") continue;
  if (p.skip) { console.log(`skip[${p.skip}] live=${p.realLive || "?"} ${file.split("/").pop()}`); continue; }
  acted++;
  if (MIN_MTIME > 0) {
    const ageSec = (Date.now() - statSync(file).mtimeMs) / 1000;
    if (ageSec < MIN_MTIME) { console.log(`ACTIVE skip (age ${Math.round(ageSec)}s) live=${p.realLive} ${file.split("/").pop()}`); continue; }
  }
  const what = [];
  if (p.trimRec) what.push(`TRIM ${Math.round(p.realLive / 1000)}K->~${Math.round(p.effectiveLive / 1000)}K @${p.boundaryTs}`);
  if (p.usageEdit) what.push(`USAGE ${p.usageEdit.from.toLocaleString()}->${p.usageEdit.to.toLocaleString()}`);
  console.log(`${what.join("  ")}  ${file.split("/").pop()}`);
  if (APPLY) {
    try { const bak = apply(file, p); healed++; console.log(`   APPLIED backup=${bak}`); }
